...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="The comma separated list of languages you want to leave to perform zero shot learning"
        + "If no language is provided then the model will fine tune on all 9 languages",
    )
    parser.add_argument("--train_on_languages", default = None, type =str, help = "Comma seperated list of languages you want to train on. Will train on everything if not used")
    parser.add_argument("--ensemble", action="store_true", help="Whether to ensemble or not.")
    parser.add_argument("--ensemble_models_path", default=None, type=str, help="The path to the directory which contains all ensemble models.")

    parser.add_argument("--freeze_upto",default=None,type=str,help="")
    ...
    if args.freeze_upto is not None:
        for name,param in model.named_parameters():
            ...
            name = name.split('.')
            if len(name) > 4 and name[3].isdigit() and int(name[3]) <= int(args.freeze_upto):
                param.requires_grad=False
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        if args.do_train:
            ...
            if not args.ensemble:
                ...
                checkpoints = [model_file for model_file in glob.glob(args.ensemble_models_path + "*/")]
                print("Models being ensembled : \n", checkpoints)
                result = evaluate_ensemble(args, checkpoints, tokenizer, model_class)
